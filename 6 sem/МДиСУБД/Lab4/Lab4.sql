...
IF v_query_type = 'SELECT' THEN DBMS_OUTPUT.PUT_LINE('SELECT statement');
...
ELSIF v_query_type = 'DELETE' THEN DBMS_OUTPUT.PUT_LINE('DELETE statement');
SELECT json_value(json_str, '$.table') INTO v_tables
FROM dual;
...
END IF;
EXECUTE IMMEDIATE v_sql;
ELSIF v_query_type = 'UPDATE' THEN DBMS_OUTPUT.PUT_LINE('UPDATE statement');
SELECT json_value(json_str, '$.table') INTO v_tables
FROM dual;
...
EXECUTE IMMEDIATE v_sql;
ELSIF v_query_type = 'CREATE TABLE' THEN DBMS_OUTPUT.PUT_LINE('CREATE TABLE statement');
SELECT json_value(json_str, '$.table') INTO v_tables
FROM dual;
...
IF v_pks IS NOT NULL THEN v_sql := v_sql || ', ' || v_pks;
END IF;
IF v_fks IS NOT NULL THEN v_sql := v_sql || ', ' || v_fks;
END IF;
v_sql := v_sql || ')';
...
-- 
BEGIN --     parse_json_proc(json_test_join_select, output_cursor);
